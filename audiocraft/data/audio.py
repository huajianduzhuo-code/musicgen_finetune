...
def _av_read(filepath: tp.Union[str, Path], seek_time: float = 0, duration: float = -1.) -> tp.Tuple[torch.Tensor, int]:
    ...
    """
    _init_av()
    # print("看看_av_read参数, filepath=",filepath,"seek_time=",seek_time,"duration=",duration)
    with av.open(str(filepath)) as af:
        stream = af.streams.audio[0]
...
def audio_read(filepath: tp.Union[str, Path], seek_time: float = 0.,
    ...
    # print("Function 'audio_read' was called")
    # stack = traceback.extract_stack()
    # # The second to last entry in the stack trace is the caller of this function
    # caller = stack[-2]
    # print(f"Called from file: {caller.filename}, line: {caller.lineno}, function: {caller.name}")
